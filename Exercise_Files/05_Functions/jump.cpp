...
int main() {
    while (jump(prompt()))  // calls prompt() and passes its return value to jump(), until jump() returns false (0).
        continue;
    puts("\nDone.");
    return 0;
}
...
int jump(const char * rs) {
    ...
    int func_length = sizeof(funcs) / sizeof(funcs[0]);  // there are 5 items in the array
    ...
    if ( i < 1 || i > func_length ) {
        ...
        funcs[i - 1]();         // Remember that C++ arrays are zero-based. Call the corresponding function from the array of function_ptrs
        return 1;
    }
}
