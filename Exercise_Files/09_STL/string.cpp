...
int main() {
    ...
    // The iterator has been used in the previous paragraph and needs to be set again
    it = s1.begin() + 5;
    s1.erase(it);
    cout << "after erase: " << s1 << endl;

    // replace
    s1.replace(5, 2, "ain't");  // At position 5, remove 2 characters and insert the string in the 3rd argument
    cout << "after replace: " << s1 << endl;

    // substr
    cout << "substr: " << s1.substr(5, 5) << endl;  // Return the string in 's1' starting from position 5, and 5 characters long
