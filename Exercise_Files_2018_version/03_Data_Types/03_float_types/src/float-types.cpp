...
int main() {
    float f;
    double df;
    long double ldf;  // 128 bits are reserved; but only 80 bits are used (certainly not available on MS Visual Studio 2017 where it is still 64 bits)
    ...
    /* df = 5e20; */
    /* df = .1 + .1 + .1;  // Floating point numbers give you scale, but at the expense of precision */
    ldf = .1 + .1 + .1;  // Floating point numbers give you scale, but at the expense of precision
    if ( ldf == .3 ) {
        puts("True");
    } else {
        puts("False");
...
// On most modern systems, an 80 bit float the precision varies, but still not infinite
