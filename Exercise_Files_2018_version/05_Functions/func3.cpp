...
const string& func_ret_strref() {
    const static string s =
        "This is func()";  // static because automatic variables are stored on
                           // the stack and we don't want to store a big string
                           // on the stack for performance reasons. 'const' is
                           // always a good idea especially with static storage,
                           // especially when passing around references.
                           // Generally good idea to use as default for anything
    return s;
}
