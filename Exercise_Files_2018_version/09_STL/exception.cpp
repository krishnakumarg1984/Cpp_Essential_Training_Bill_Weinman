...
// The standard exception class is designed to declare objects to be thrown as exceptions
// The exception class has a virtual function called 'what' that returns a
// C-string. This function can be overloaded in derived classes to provide a
...
class MyExceptionClass : public exception {
    ...
    MyExceptionClass();
 public:
    MyExceptionClass(const char * s) throw() : msg(s) {}  // Initialise a message in this constructor
    const char * what() const throw() { return msg; }  // Overloaded what() to return that message
};
