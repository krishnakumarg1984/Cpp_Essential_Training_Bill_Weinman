...
int main() {
    ...
    ifstream infile(
        filename);  // Open an input filestream. ifstream is the class. infile
                    // is the object name that we have chosen
    while (infile.good()) {
        infile.getline(buf, sizeof(buf));
        cout << buf << endl;  // Display the line contents read from the file
                              // onto the screen
    }
    infile.close();
